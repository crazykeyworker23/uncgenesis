...
export const DevotionalList: React.FC = () => {
  ...
  const { data, isLoading, isError, error } = useQuery({
    queryKey: ['devotionals', search, statusFilter, page],
    queryFn: async () => {
  ...
  const publishMutation = useMutation({
    mutationFn: (id: number) => apiClient.post(`/devotionals/${id}/publish/`),
    onSuccess: () => {
  ...
  const archiveMutation = useMutation({
    mutationFn: (id: number) => apiClient.post(`/devotionals/${id}/archive/`),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['devotionals'] });
    }
  });
  ...
  const duplicateMutation = useMutation({
    mutationFn: (id: number) => apiClient.post(`/devotionals/${id}/duplicate/`),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['devotionals'] });
    }
  });
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: number) => apiClient.delete(`/devotionals/${id}/`),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['devotionals'] });
    }
  });

  const getStatusBadge = (status: DevotionalStatus) => {
    switch (status) {
      case 'PUBLISHED':
    ...
    <div className="space-y-6">
      ...
      <div className="glass-panel p-4 bg-dark-teal bg-opacity-20 flex flex-col md:flex-row gap-4 items-center">
        ...
        <div className="flex items-center gap-2 px-3 py-2 bg-deep-teal bg-opacity-40 border border-white border-opacity-10 rounded-xl w-full md:w-80 text-xs">
          ...
          <input 
            type="text" 
            placeholder="Buscar por título, pasaje..." 
            value={search}
            onChange={(e) => {
      ...
      {isError && (
        <div className="flex items-center gap-2 p-3 bg-error-red bg-opacity-15 text-error-red border border-error-red border-opacity-20 rounded-xl text-xs">
          <AlertCircle size={16} />
          <span>Error al cargar los devocionales: {(error as any)?.message}</span>
        </div>
      )}
      ...
      <div className="glass-panel bg-dark-teal bg-opacity-15 overflow-hidden">
        {isLoading ? (
          <div className="p-12 flex flex-col items-center justify-center gap-3">
            ...
            <span className="text-xs text-crema text-opacity-50">Cargando devocionales...</span>
          </div>
        ) : !data?.results || data.results.length === 0 ? (
          <div className="p-12 text-center">
            <span className="text-xs text-crema text-opacity-40">No se encontraron devocionales registrados.</span>
          </div>
        ) : (
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-white border-opacity-5 text-[10px] font-bold uppercase tracking-wider text-crema text-opacity-40 bg-deep-teal bg-opacity-20">
                  <th className="px-6 py-4">Fecha Devocional</th>
                  <th className="px-6 py-4">Título</th>
                  <th className="px-6 py-4">Pasaje Bíblico</th>
              ...
              <tbody className="divide-y divide-white divide-opacity-5 text-xs">
                {data.results.map((dev: Devotional) => (
                  <tr key={dev.id} className="hover:bg-white hover:bg-opacity-5 transition-colors">
                    {/* Date */}
                    <td className="px-6 py-4 text-crema text-opacity-70 font-semibold">
                      <div className="flex items-center gap-1.5">
                        ...
                        {new Date(dev.date).toLocaleDateString('es-PE')}
                      </div>
                    </td>

                    {/* Title */}
                    <td className="px-6 py-4 font-bold text-crema max-w-xs truncate">
                      <span title={dev.title}>{dev.title}</span>
                    </td>

                    {/* Bible Passage */}
                    <td className="px-6 py-4 text-crema text-opacity-70 font-medium">
                    ...
                    <td className="px-6 py-4">
                      {dev.audio_url ? (
                        ...
                        <span className="text-crema text-opacity-20">-</span>
                      )}
                    </td>
                    ...
                    <td className="px-6 py-4 text-crema text-opacity-55">
                      {dev.author?.full_name || 'Desconocido'}
                    </td>
                      ...
                      <td className="px-6 py-4 text-right">
                        <div className="flex items-center justify-end gap-1.5 whitespace-nowrap">
                          ...
                          {dev.status !== 'PUBLISHED' && (
                            <button
                              onClick={() => publishMutation.mutate(dev.id)}
                          ...
                          {dev.status === 'PUBLISHED' && (
                            <button
                              onClick={() => archiveMutation.mutate(dev.id)}
                          ...
                          <button
                            onClick={() => duplicateMutation.mutate(dev.id)}
        ...
        {data?.count && data.count > 10 && (
          <div className="flex items-center justify-between px-6 py-4 border-t border-white border-opacity-5 bg-deep-teal bg-opacity-10 text-xs">
            <span className="text-crema text-opacity-40">
              Mostrando {data.results.length} de {data.count} devocionales
            </span>
